...
usage() {
  cat <<'USAGE'
Usage:
  bash batch-jobs/yahoo-daily-bars-data/update-sp500-eod-safe.sh [options]
...
Options:
  ...
  --log-root PATH              Host log root. Default: LOG/showmeedge-<universe>-eod
...
Examples:
  ...
  bash batch-jobs/yahoo-daily-bars-data/update-sp500-eod-safe.sh --universe forex_major_pairs --allow-small-universe --rebuild
USAGE
}
...
RUN_LABEL="$(run_label "$UNIVERSE" "eod")"
if [[ -z "$LOG_ROOT" ]]; then
